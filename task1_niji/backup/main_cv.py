...
if skl:
    ...
    estimator3 = KernelRidge(kernel='polynomial',degree=4)	
    estimator3.fit(x_train,y_train)
    y_predict3 = estimator3.predict(x_validation)

    lasso = Lasso(random_state=2,normalize=True)
    ...
    #for alpha in alphas:
   	# estimator3.alpha = alpha
   	# this_scores = cross_val_score(estimator3, x_train, y_train, cv=n_folds, n_jobs=1)
   ...
   # print(max(scores))
    ...
    estimator3.alpha=2.68269580
    rms=0
    number=0
    weights=np.zeros(855)
    for train_index, test_index in kf.split(x_training):
       ...
       estimator3.fit(x_train,y_train)
       y_predict1=estimator3.predict(x_validation)
       RMSE = mean_squared_error(y_validation,y_predict1) ** 0.5
       print(RMSE)

       if(RMSE<20):
       		number +=1
		weights+=estimator3.dual_coef_	
		print(weights)
       #error_calculation(y_predict5,y_validation)
    opt_weights=weights/(number)
    print(opt_weights)
    print(number)
    estimator3.dual_coef_=opt_weights
...
output1 = np.hstack((test_data_Id,y_test3))
